...
# ===== 한글 폰트(1회 로딩) =====
FONT_PATH = "C:/Windows/Fonts/malgun.ttf"  # 환경에 맞게
KOREAN_FONT = ImageFont.truetype(FONT_PATH, 30)

def draw_text_korean(frame, text, position, font_size=30, color=(255,255,255)):
    # 이미 로딩된 폰트를 크기별로 다시 만들고 싶다면 캐시를 쓰거나 아래 한 줄만 교체
    font = KOREAN_FONT if font_size == 30 else ImageFont.truetype(FONT_PATH, font_size)
...
print("🔁 실시간 수어 예측 시작 (슬라이딩 10프레임, 'q'로 종료)")
...
with mp_holistic.Holistic(
    ...
    while cap.isOpened():
        ...
        if results.left_hand_landmarks is None and results.right_hand_landmarks is None:
            ...
            confidence = 0.0
            # print(f"[SKIP] 양손 없음 → None 출력")  # 콘솔 과도 출력이면 주석
        ...
        # 🔹 랜드마크 추출 (학습과 동일 구성: LH(63)+RH(63)+POSE(17*4))
        ...
        # 🔹 예측: 학습과 분포 일치 위해 '정규화 제거'
        if len(frame_buffer) == BUFFER_SIZE:
            input_data = np.array(frame_buffer, dtype=np.float32)  # (10, 194)

            # ⚠️ 학습 시 per-sample 정규화가 없었으므로 제거합니다.
            # max_abs = np.max(np.abs(input_data))
            # if max_abs > 0:
            #     input_data = input_data / max_abs

            input_data = np.expand_dims(input_data, axis=0)  # (1, 10, 194)
        ...
        display_frame = draw_text_korean(
            ...
            (10, 30),
            font_size=32,
            color=(0,255,0) if predicted_label != "none" else (0,0,255)
        )
        display_frame = draw_text_korean(
            display_frame,
